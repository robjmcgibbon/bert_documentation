...
def create_virtual_snapshot(input_filename, force=False, verbose=False):
    # parse the input file name
    try:
        prefix, file_nr = filename_re.match(input_filename).groups()
    except:
        raise RuntimeError(
            f"Could not decompose input filename {input_filename}."
    ...
    try:
        with h5py.File(input_filename, "r") as handle:
        ...
        raise RuntimeError(f"Cannot open file {input_filename}!")
    ...
    try:
        with h5py.File(input_filename, "r") as ifile, h5py.File(
            output_filename, "w"
        ) as ofile:
            for group in ifile.keys():
                if (
                    parttype_re.match(group) is None
                    and partname_re.match(group) is None
                ):
                    ifile.copy(group, ofile)
    except:
        raise RuntimeError(
            "Something went wrong while trying to copy over non-particle groups from"
            f" {input_filename} to {output_filename}!"
        )
    ...
    try:
        with h5py.File(output_filename, "r+") as handle:
            handle["/Header"].attrs["NumFilesPerSnapshot"] = np.array(
                [1], dtype=np.int32
            )
    ...
    # 0we first gather these offsets (they are the same as the dataset offsets)
    particle_offsets = {}
    for group in particle_groups:
        for dset in particle_groups[group]:
    ...
    try:
        with h5py.File(output_filename, "r+") as handle:
            for group in particle_offsets:
                ...
                handle[f"Cells/OffsetsInFile/{group}"][:] += particle_offsets[group][
                    files
                ]
...
if __name__ == "__main__":
    ...
    create_virtual_snapshot(args.input, args.force, verbose=True)
